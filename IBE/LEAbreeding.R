...
nameList <- c("FORMAT",breedingInds$FileName)
...
##filtering VCF by polymorphic SNPs
polySNPs <- is.polymorphic(allInds)
polySNPs[is.na(polySNPs)] <- FALSE  

newVCF <- allInds

newVCF <- newVCF[polySNPs,]
write.vcf(newVCF, "gonadOnlyFiltered.vcf.gz")


##Importing and converting data
#vcf2lfmm("m3MAC1Unlinkgq30.vcf")
#vcf2geno("m3MAC1Unlinkgq30.vcf")
vcf2lfmm("gonadOnlyFiltered.vcf")
vcf2geno("gonadOnlyFiltered.vcf")
...
write.env(breedingenvData, "breedingGonadclimVar.env")
...
project = NULL
project = snmf("gonadOnlyFiltered.geno",
               #               K = 1:6, 
               K=3,
               entropy = TRUE, 
               repetitions = 10,
...
## ----results='hide'-----------------------------------------------------------
project.missing = snmf("gonadOnlyFiltered.lfmm", K = 3, entropy = TRUE, repetitions = 10,project = "new")
...
impute(project.missing, "gonadOnlyFiltered.lfmm", method = 'mode', K = 3, run = best)


# Proportion of correct imputation results
dat.imp = read.lfmm("gonadOnlyFiltered.lfmm_imputed.lfmm")
...
project = NULL
project = lfmm("gonadOnlyFiltered.lfmm", 
               "breedingGonadclimVar.env", 
               K = 3, 
               repetitions = 5, 
               project = "new")
...
# Proportion of correct imputation results
dat.imp = read.lfmm("gonadOnlyFiltered.lfmm")


## -----------------------------------------------------------------------------
# Fitting an LFMM with K = 3 factors
mod <- lfmm2("gonadOnlyFiltered.lfmm", "breedingGonadclimVar.env", K = 3)

## ----fig.width=8, fig.height=6, echo=TRUE, results='hide'---------------------
# Computing P-values and plotting their minus log10 values 
pv <- lfmm2.test(object = mod, "gonadOnlyFiltered.lfmm", "breedingGonadclimVar.env", linear = TRUE)
